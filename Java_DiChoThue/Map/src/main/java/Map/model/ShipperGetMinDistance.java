...
public class ShipperGetMinDistance {
	@Autowired
	...
	String uri = "http://dev.virtualearth.net/REST/V1/Routes?wp.0=" + mywaypoint + "&wp.1=" + otherwaypoint + "&key=" + bingMapKey;
	String jsonBingMap;
	Float distance;

	public ShipperAndAPIDistances findMin(List<ShipperAndAPIDistances> list)
    {
		ShipperAndAPIDistances maxDistance = list
			      .stream()
	...
	public List<ShipperAndAPIDistances> caculateMinDirection() {
		...
		for(Shipper shipper : shipperlst) {
			...
			//parse thành json
			Gson gson = new Gson();
			JsonObject jsob = gson.fromJson(jsonBingMap, JsonObject.class);
			jsob = (JsonObject) jsob.get("resourceSets");
			jsob = (JsonObject) jsob.get("resources");
			distance = jsob.get("travelDistance").getAsFloat();
